...
#Assign a strength score (Weak, Moderate, Stronge).
...
#Moderate (Score: 3-4) → Good but missing key elements
#Strong (Score: 5) → Meets all criteria

#3. Feedback System

#If the password is weak, suggest improvements.
#If the password is stronge, display a sucess message.

import re
import streamlit as st

#page styling
st.set_page_config(page_title="Password Strength Checker By Areej Shah", page_icon="🌘", layout="centered")
#custom css
st.markdown(""" 
<style>
    ...
    .stButton button:hover {background-color: pink}
</style>
...
#function to cheack password strength
def check_password_strength(password):
    ...
    if re.search(r"[A-Z]", password) and re.search(r"[a-z]", password):
        ...
        feedback.append("❌ Password should include **both uppercase (A-Z) and lowcase (a-z) letters**.")
    ...
    if feedback:
        with st.expander("🔍 **Improve Your Password** "):
            for item in feedback:
                st.write(item)
password = st.text_input("Enter your password:", help="Ensure your password is strong 🔐")
